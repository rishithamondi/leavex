...
const AdminLeaveCalendar: React.FC = () => {
  ...
  const fetchLeaves = async () => {
    try {
      ...
      setLeaves(data as LeaveWithStudent[]);
    } catch (error) {
  ...
  const getLeavesForDate = (d: Date): LeaveWithStudent[] => {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 space-y-4 md:space-y-0">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            View all students&apos; active, pending, approved, and rejected leave applications in a monthly calendar.
          </p>
        </div>
      ...
      {/* Calendar Controls */}
      <div className="bg-white rounded-t-xl border-t border-l border-r border-gray-200 p-4 flex items-center justify-between shadow-sm">
        ...
        <button
          ...
          className="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-gray-900 transition-colors shadow-sm"
        >
          Today
        </button>
      </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col gap-0.5 w-full mt-auto">
                  {approvedCount > 0 && (
                    <div className="flex items-center text-[9px] md:text-[10px] font-semibold text-green-700 bg-green-50/80 px-1 py-0.5 rounded border border-green-100 max-w-full truncate" title={`${approvedCount} Approved`}>
                      <CheckCircle2 size={10} className="mr-1 text-green-600 flex-shrink-0" />
                      <span>{approvedCount} <span className="hidden md:inline">Approved</span></span>
                    </div>
                  )}
                  {pendingCount > 0 && (
                    <div className="flex items-center text-[9px] md:text-[10px] font-semibold text-yellow-700 bg-yellow-50/80 px-1 py-0.5 rounded border border-yellow-100 max-w-full truncate" title={`${pendingCount} Pending`}>
                      <Clock3 size={10} className="mr-1 text-yellow-600 flex-shrink-0" />
                      <span>{pendingCount} <span className="hidden md:inline">Pending</span></span>
                    </div>
                  )}
                  {rejectedCount > 0 && (
                    <div className="flex items-center text-[9px] md:text-[10px] font-semibold text-red-700 bg-red-50/80 px-1 py-0.5 rounded border border-red-100 max-w-full truncate" title={`${rejectedCount} Rejected`}>
                      <CircleX size={10} className="mr-1 text-red-600 flex-shrink-0" />
                      <span>{rejectedCount} <span className="hidden md:inline">Rejected</span></span>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-gray-900">
              Leave Details for {selectedDate.toLocaleDateString(undefined, { weekday: 'long', month: 'short', day: 'numeric', year: 'numeric' })}
            </h3>
          </div>
              ...
              <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-3">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border flex items-center gap-1 ${statusColorClass}`}>
                  ...
                  {leave.students && (
                    <div className="space-y-1">
                      <h4 className="text-base font-bold text-gray-900">{leave.students.name}</h4>
                      <p className="text-xs text-gray-500">
                        Reg No: <span className="font-semibold text-gray-700">{leave.students.reg_no}</span> · 
                        Room: <span className="font-semibold text-gray-700">{leave.students.hostel_room_no}</span> · 
                        Phone: <span className="font-semibold text-gray-700">{leave.students.phone}</span>
                      </p>
                    </div>
                  )}
                ...
                <div className="flex-shrink-0 self-start md:self-center">
                  <button
                    onClick={() => router.push(`/admin/leaves/${leave.id}`)}
          ...
          return (
            <div className="p-5 max-h-[380px] overflow-y-auto space-y-6 scrollbar-thin">
              {['pending', 'accepted', 'rejected'].map((status) => {
                ...
                const statusColorClass = status === 'accepted' ? 'text-green-700 bg-green-50 border-green-200' : status === 'rejected' ? 'text-red-700 bg-red-50 border-red-200' : 'text-yellow-700 bg-yellow-50 border-yellow-200';

                return (
                  <div key={status} className="space-y-3">
                    <h4 className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-md border flex items-center gap-1 ${statusColorClass}`}>
                      {status === 'accepted' ? (
                        <CheckCircle2 size={10} className="flex-shrink-0" />
                      ) : status === 'rejected' ? (
                        <CircleX size={10} className="flex-shrink-0" />
                          ...
                          >
                            <div>
                              <div className="flex justify-between items-start">
                                <div>
                                  <p className="font-bold text-gray-900 text-sm">{leave.students?.name || 'Student'}</p>
                                  <p className="text-xs text-gray-500">{leave.students?.reg_no} · Room {leave.students?.hostel_room_no}</p>
                                </div>
